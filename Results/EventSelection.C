...
void EventSelection(){
	...
	addMCHistogram("analyzeSUSY1m_leptonSelection/Muon0_Pt",  "p_{T}^{lepton} [GeV]","Events",  0,  500, 0.1, 1e4, -0.1, 2.1, 1);
...
// 	addMCHistogram("analyzeSUSY1e_leptonSelection/Electron0_Pt",  "p_{T}^{lepton} [GeV]","Events",  0,  500, 0.1, 1e4, -0.1, 2.1, 1);
	...
	for(int hdx=0; hdx<(int)Histograms.size(); ++hdx)
	  {
	    ...
	    TText *text=label->AddText("4.98 fb^{-1}, #sqrt{s} = 7 TeV, muon channel");
	    label->Draw();
